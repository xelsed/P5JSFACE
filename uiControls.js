class UIControls {
  constructor(callbacks = {}) {
    ...
    this.sliceDetailEl = document.getElementById('slice-detail');
  ...
  setSliceInfo(count, slices = []) {
    ...
    if (this.sliceDetailEl) {
      if (!slices.length) {
        this.sliceDetailEl.textContent = 'Awaiting captured mesh…';
      } else {
        this.sliceDetailEl.innerHTML = slices
          .map((slice, idx) => `<div>Slice ${idx + 1}: ${slice.length} pts</div>`)
          .join('');
      }
    }
  }
